...
async function askOnceStream(question, model) {
    ...
    const estimateChars = (msgs) => msgs.reduce((sum, m) => sum + (typeof m.content === 'string' ? m.content.length : 0), 0);
    const buildSendMessages = (aggressive) => {
    ...
    const isServerError = (e) => {
        ...
        if (err?.response?.status && err.response.status >= 500)
            return true;
        if (typeof err?.message === 'string' && /status code 5\d\d/.test(err.message))
            return true;
        return false;
    };
    const runStream = async (sendMsgs) => {
        raw = '';
        emitted = 0;
        await (0, client_1.callAI_Stream)(sendMsgs, model, (chunk) => {
            raw += chunk;
    ...
    try {
        ...
        try {
            ...
            if (isServerError(err) && fullMessages.length > 1) {
                console.log(chalk_1.default.yellow('⚠️ 上游返回 5xx（代理未到达模型后端），1.5s 后已自动精简上下文重试'));
                await sleep(1500);
                try {
                    await runStream(buildSendMessages(true));
                }
                catch (retryErr) {
                    throw new Error(`上游代理持续返回 5xx（请求未到达模型后端）。请检查代理的上下文长度 / 请求体大小限制，` +
                        `或临时用短对话规避。原始错误：${retryErr?.message ?? String(retryErr)}`);
                }
            }
            else {
                throw err;
        ...
        const storedAssistant = fullResponse.length > MAX_ASSISTANT_STORE
            ...
            : fullResponse;
        (0, client_1.addToConversationHistory)('user', question);
        (0, client_1.addToConversationHistory)('assistant', storedAssistant);
    }
    catch (error) {
