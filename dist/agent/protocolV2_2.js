...
exports.buildChatProtocolPrompt = buildChatProtocolPrompt;
...
`;
    // 添加详细的工具定义
    const toolDefinitions = (0, toolCapability_1.generateToolDefinitionsPrompt)();
    // 重要提示
 ...
 */
function buildChatProtocolPrompt() {
...
1. **默认直接回答，不要调用工具。** 对能用自己知识回答的问题，直接输出 JSON 回答，不调任何工具，
   ...
   - 严格满足用户明确要求的篇幅/字数/条数：若要求"不少于 N 字""详细介绍""分 N 点"，必须给出充分展开、详尽完整的长回答，不要简短敷衍。
