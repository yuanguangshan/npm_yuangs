...
function extractToolCalls(text: string): Array<{ name: string; arguments: Record<string, unknown> }> {
...
function translateToOpencodeMessages(messages: any[]): Array<{ role: string; content: string }> {
...
class SimpleEventStream implements AsyncIterable<any> {
  private events: any[] = [];
  private endPromise: Promise<any>;
  private resolveEnd!: (value: any) => void;

  constructor() {
    this.endPromise = new Promise<any>((resolve) => {
...
function buildAssistantMessage(model: any, contentBlocks: any[], stopReason: string, usage: any): any {
  return {
    role: 'assistant',
...
export async function streamOpencode(
  ...
  try {
    ...
    const stdToolCalls: Array<{ id?: string; name: string; arguments: Record<string, unknown> }> = [];
    ...
    const allCalls: Array<{ id?: string; name: string; arguments: Record<string, unknown> }> = [
      ...stdToolCalls,
    ...
    const toolCallBlocks: any[] = allCalls.map((call) => ({
      type: 'toolCall',
