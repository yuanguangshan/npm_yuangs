...
请输出修正后的完整方案。`
                ...
                const costProfile = defaultCostProfileCalculator.calculate(allFiles, allFiles.length * 100);
...
同时，需要为每个主要任务标注能力等级（Capability Level）：
...
- [ ] 实现用户认证 [SEMANTIC]
  ...
  - fallbackChain: [STRUCTURAL, LINE, TEXT, NONE]`
                ...
                const metadataLines = [
                    ...
                    `> 🎯 Context: ${userPrompt}`,
                    `> 🔧 Capability Level: ${CapabilityLevel[costProfile.requiredCapability]}`,
