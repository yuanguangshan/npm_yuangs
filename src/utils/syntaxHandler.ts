...
export async function handleSpecialSyntax(
  ...
  if (trimmed.startsWith("#")) {
    ...
    if (spaceMatch) {
      ...
      const diskExists = (() => {
        try {
          return fs.statSync(candidateFullPath).isDirectory();
        } catch {
          return false;
        }
      })();
 ...
 */
function parseCatSpec(spec: string): {
  index: number | null;
...
async function handleDirectoryReference(
  dirPath: string,
  question?: string,
): Promise<{
  processed: boolean;
...
async function handleAtomicExec(
  command: string,
): Promise<{ processed: boolean; result: string }> {
...
async function handleListContext(): Promise<{
  processed: boolean;
  result: string;
}> {
  try {
...
async function handleClearContext(): Promise<{
  processed: boolean;
  result: string;
}> {
  try {
